...
# imagePullPolicy, and the three ERROR_INJECTION_* env vars in-stream.
# The source file is never modified (no `sed -i`), so re-running the
# script is safe and `git status` stays clean.
#
# Usage:
...
apply_local_job() {
  ...
  # from any registry, and override the error-injection env vars.
  # The ERROR_INJECTION_* lines are pairs (- name: / value: "..."), so
  # we use sed's `N` to read the next line into the pattern space, then
